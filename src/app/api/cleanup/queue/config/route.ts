...
import { DEFAULT_FAILED_IMPORT, FailedImportConfig, QueueCleanerConfigShape } from '@/lib/cleanup/types';
...
function validateConfig(body: unknown): { ok: true; value: QueueCleanerConfigShape } | { ok: false; error: string } {
  ...
  if (!Number.isFinite(intervalMinutes) || intervalMinutes < 1) {
    return { ok: false, error: 'intervalMinutes must be >= 1' };
  }
...
async function putHandler(req: NextRequest) {
  ...
  const previousInterval = (await loadQueueCleanerConfig()).intervalMinutes;
  await saveQueueCleanerConfig(result.value);
  if (result.value.intervalMinutes !== previousInterval || true) {
    // Always restart so toggling enabled also takes effect
