...
import { Search, ArrowLeft, Plus, Loader2, Tv } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        <Input placeholder="Search for a TV series..." value={term} onChange={(e) => setTerm(e.target.value)} className="flex-1" />
      ...
      {selected ? (
          ...
          <CardContent className="pt-4 space-y-4">
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{selected.year} &middot; {selected.network}</p>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1">
                ...
                <Select value={rootFolder} onValueChange={setRootFolder}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {rootFolders.map((f) => <SelectItem key={f.id} value={f.path}>{f.path}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1">
              ...
              <div className="flex items-center gap-4 pt-5">
                ...
                <div className="flex items-center gap-2">
                  <Switch checked={seasonFolder} onCheckedChange={setSeasonFolder} id="season-folder" />
                  <Label htmlFor="season-folder">Season Folders</Label>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {searching
            ...
            : results.map((r) => (
                <button key={r.tvdbId} onClick={() => setSelected(r)} className="text-left group">
                  <div className="relative aspect-[2/3] rounded-lg overflow-hidden bg-muted">
                    {posterUrl(r.images as { coverType: string; remoteUrl: string }[]) ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center"><Tv className="h-8 w-8 text-muted-foreground" /></div>
                    )}
