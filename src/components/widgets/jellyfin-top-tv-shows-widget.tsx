...
async function fetchTopTv({ days, userId, signal }: { days: number; userId: string; signal?: AbortSignal }): Promise<PlaybackBreakdownEntry[]> {
  const params = new URLSearchParams({ days: String(days) });
  if (userId) params.set('userId', userId);
  const res = await fetch(`/api/jellyfin/playback/tv-shows?${params}`, { signal });
