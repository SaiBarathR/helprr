...
export const searchTmdb: ProviderHandler = async ({ query, limit }) => {
  try {
    ...
    // If the provider fails first (for example, a rate limit), keep the
    // concurrently-started cache warm-up from becoming an unhandled rejection.
    ...
    const { movies, series } = await libraryPromise;
